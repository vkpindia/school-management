...
export class TeachersService {
  ...
  /**
   * @description Method for getting all teachers data from server
   * @author Virendra Pandey
   * @date 2020-06-21
   * @returns
   * @memberof teachersService
  ...
  getAllTeacher() {
    ...
    return this._http.get(this._apiUrl + 'teacher').pipe(
      map(this.extractData),
      catchError(this.handleError));
  }

  /**
   * @description Method for getting unique teacher data by key
   * @author Virendra Pandey
   * @date 2020-06-21
   * @param {*} id
  ...
  getTeacherDetails(id: any): Observable<any> {
    const url = `${this._apiUrl}teacher/GetTeacher?id=${id}`;
  ...
  /**
   * @description Method for saving teacher data in server
   * @author Virendra Pandey
   * @date 2020-06-21
   * @param {*} users
   * @returns {Observable<any>}
   * @memberof teachersService
   */
  postTeacher(teacherRecord: any)/* : Observable<any> */ {
    console.log('teacherRecord', teacherRecord);
    return this._http.post(this._apiUrl + 'teacher/AddTeacher', teacherRecord); //
    /*  .pipe(
       map(this.extractData),
       catchError(this.handleError)
     ); */
  ...
  /**
   * @description Method to Update teacher record
   * @author Virendra Pandey
   * @date 2020-06-21
   * @param {*} users
   * @returns {Observable<any>}
   * @memberof teachersService
   */
  updateTeacher(id, teacherRecord: any): Observable<any> {
    return this._http.put(this._apiUrl + 'teacher/UpdateTeacher', teacherRecord)
      .pipe(
        catchError(this.handleError)
      );
  }
