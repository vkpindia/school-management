...
export class ExamResultComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['select', 'examname', 'examdate', 'actualmarks', 'totalmarks',  'actions'];
   ...
   */
  public getResults() {
    this.as.getTotalResultsByStudentID(this.examID.value, JSON.parse(localStorage.getItem('currentUser')).id).subscribe(data => {
  ...
  public onDelete(row): void {
    let isDelete: boolean = confirm("Are sure you want to delete this result?");
    if (isDelete) {
      this.as.deleteResults(row.id).subscribe(data => {
        this.getResults();
      });
    }
  }
