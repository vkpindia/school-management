...
@Component({
  selector: 'app-approval-level-second',
  templateUrl: './approval-level-second.component.html',
  styleUrls: ['./approval-level-second.component.scss']
})
export class ApprovalLevelSecondComponent implements OnInit {
  ...
  public getFirstLevelRegistartion() {
    this._ss.getRegistrationSecondInfo().subscribe((data: any) => {
  ...
  public onApprove(row): void {
    if (row) {
      this._ss.updateRegistartionInfo(row.id, true, 1).subscribe(data => {
  ...
  public onDecline(row): void {
    if (row) {
      this._ss.updateRegistartionInfo(row.id, false, 1).subscribe(data => {
        this.showNotification('Request Approved');
      });
    }
  }
